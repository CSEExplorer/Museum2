...
const MuseumDashboard = ({ uniqueId }) => {
   const { museumData,setMuseumData } = useMuseum();
    ...
    const fetchMuseumDetails = async () => {
        try {
            const response = await axios.post(`${apiUrl}/museums/museum-details/`, { uniqueId });
    ...
    return (
        <div className="container mt-5" style={styles.container}>
            <h1 className="text-center mb-4">Welcome to {museumData.name}'s Dashboard</h1>
            <div className="row">
                {/* Museum details */}
                <div className="col-md-12 mb-4">
                    <div className="card" style={styles.card}>
                        <div className="card-body">
                            ...
                            <p><strong>Name:</strong> {museumData.name}</p>
                            <p><strong>Address:</strong> {museumData.address}</p>
                            <p><strong>City:</strong> {museumData.city}</p>
                            <p><strong>Email:</strong> {museumData.email}</p>
                            <p><strong>Contact Number:</strong> {museumData.contact_number}</p>
                            <p><strong>Fare:</strong> {museumData.fare}</p>
                            <p><strong>Created At:</strong> {new Date(museumData.created_at).toLocaleDateString()}</p>
                            <Link to="/update-details" className="btn btn-primary">Update Museum Info</Link>
                        </div>
                    </div>
                </div>
